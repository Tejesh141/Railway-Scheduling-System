      ...
      <div className="p-4 space-y-3 max-h-96 overflow-y-auto">
        {loading ? (
          <div className="flex items-center justify-center py-10">
            <Loader2 size={18} strokeWidth={1.8} className="animate-spin" style={{ color: '#6B6B7B' }} />
          </div>
        ) : error ? (
          <p className="text-xs text-center py-6" style={{ color: '#6B6B7B' }}>{error}</p>
        ) : visible.length === 0 ? (
          <div className="text-center py-8">
            <CheckCircle size={28} strokeWidth={1.8} className="mx-auto mb-2" style={{ color: '#E2E0D8' }} />
            ...
            <div key={conflict.id} className="border rounded-xl p-4 transition-all duration-200" style={{ background: '#FAFAF8', borderColor: '#E2E0D8' }}>
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center space-x-2">
                  <AlertTriangle size={15} strokeWidth={1.8} style={{ color: conflict.severity === 'Critical' ? '#1A1A2E' : '#6B6B7B' }} />
              ...
              <div className="flex items-center space-x-2">
                ...
                >
                  {resolving === conflict.id ? <Loader2 size={13} strokeWidth={1.8} className="animate-spin" /> : <CheckCircle size={13} strokeWidth={1.8} />}
